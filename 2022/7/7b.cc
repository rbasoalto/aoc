...
int main(int argc, char** argv) {
  ...
  sh.ComputeSizes([](Dir* d) -> bool {return false;}, &interesting_dirs);

  const size_t fs_size = 70000000;
  const size_t fs_req = 30000000;

  const size_t free_space = fs_size - *sh.root_.size_;
  const size_t need_to_delete = fs_req - free_space;

  sh.ComputeSizes([&](Dir* d) -> bool { return *d->size_ >= need_to_delete; }, &interesting_dirs);

  size_t min_dir = fs_size;
  for (const auto* dir : interesting_dirs) {
    min_dir = std::min(min_dir, *dir->size_);
  }
  std::cout << min_dir << std::endl;
